...
class SegmentioCell: UICollectionViewCell {
    ...
    let segmentTitleLabelHeight: CGFloat = 14
    ...
    fileprivate var segmentItem : SegmentioItem?
    ...
    override init(frame: CGRect) {
        ...
        segmentImageView?.backgroundColor = UIColor.clear
    ...
    func configure(selected: Bool) {
        ...
        if style.isWithImage() && segmentItem != nil {
            segmentImageView?.image = (selected && segmentItem?.imageSelected != nil) ? segmentItem?.imageSelected : segmentItem?.image
        }
    }
