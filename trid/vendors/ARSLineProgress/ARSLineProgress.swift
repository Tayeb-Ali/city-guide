...
private let ARS_CIRCLE_ROTATION_TO_VALUE = 2 * CGFloat(Double.pi)
...
private let ARS_CIRCLE_START_ANGLE: CGFloat = -CGFloat(Double.pi/2)
...
extension ARSStatus {
    ...
    static func drawFail(_ backgroundView: UIVisualEffectView) {
        ...
        failCircle.path = UIBezierPath(arcCenter: failCircleArcCenter,
            radius: ARS_CIRCLE_RADIUS_OUTER,
            startAngle: -CGFloat(Double.pi/2),
            endAngle: CGFloat(Double.pi/2) / 180 * 270,
            clockwise: true).cgPath
...
@discardableResult fileprivate func ars_createdFrameForBackgroundView(_ backgroundView: UIView, onView view: UIView?) -> Bool {
    let center: CGPoint
